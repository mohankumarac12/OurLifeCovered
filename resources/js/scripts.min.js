...
function getQuote(demographicData, coverage, term, successCallback) {
    var fakeResponseData = {
        bestClassPrice: +numberWithCommas(Math.floor(99 * Math.random() + 1)),
        standardClassPrice: +numberWithCommas(Math.floor(99 * Math.random() + 1))
    };
    $.ajax({
        url: "http://enlivenhq.com",
        type: "POST",
        data: {
...
function getRecommendedCoverage(formData, successCallback) {
    $.ajax({
...
function submitContactInfo(email, phone, bestTimeToCall, calculatorData, needsCalculatorDemographicData, directQuoteDemographicData, coverage, term, successCallback,comments,name, firstName,dob,emailAddress,Smoker,gender,state,us,policyOwner,replacement,currentAnnualIncome,pathId) {
    ...
    $.ajax({
        ...
        data: {
            ...
            dob : dob,
            cntEmailAddress: emailAddress,
            Smoker: Smoker,
            cntGender: gender,
            cntState: state,
            us: us,
            chkPolicyOwner: policyOwner,
...
function changeSection(target) {
    var viz = $(target).is(":visible");
    viz ? $("html, body").animate({
        scrollTop: $(target).offset().top
    }, 1e3) : ($(".main-section").hide(), $(target).show(), $(target).slideDown(function() {
        $("html, body").animate({
            scrollTop: $(target).offset().top
        }, 1e3);
    })), location.hash !== target && history.pushState({}, "", target);
}

function changeAndScroll(button, target) {
//     $(button).on("click", function(e) {
//         e.preventDefault(), changeSection(target);
//     });
}
...
function knockout(message) {
    message = message || "Unfortunately, you do not qualify for our online application process, but this is far from saying that you cannot be protected! Please complete this form and and we will be in touch shortly to assist you.", 
    $("#contact-us-intro-text").text(message), changeSection("#contact-us");
}

function knockout2M(message) {
	$("#contact-us-intro-text").html(message), changeSection("#contact-us");
}


function displayErrorText(message, selector) {
    $(selector).find(".success-text").hide(), $(selector).find(".error-text").hide(), 
    $(selector).find(".error-text").html(message), $(selector).find(".error-text").slideDown(), 
    $("body, html").animate({ scrollTop: $(selector).offset().top }, 1000);
    changeAndScroll(".open-contact-form", "#contact-us");
}
...
function sendContactForm(contactForm,pathId) {
	if (pathId == '2') {
	   // Get Quote path
		var needsCalculatorDemographicData = $(document.needsCalculatorDemographicsForm).serializeObject(), calculatorData = $(document.needsCalculatorForm).serializeObject(), directQuoteDemographicData = $(document.getQuoteDemographicsForm).serializeObject(), coverage = $(".calculated-coverage:first").text().replace("$", "").replace(",", ""), term = null, email = $("#contact-form-email").val(), phone = $("#contact-form-phone").val(), bestTimeToCall = $("#contact-form-best-time").val(), comments = $("#contact-form-comments").val(),name = $("#contact-form-name").val(),firstName= $("#get-a-quote-first-name").val(),dob = $("#get-a-quote-date-of-birth").val(),emailAddress= $("#get-a-quote-email-address").val(),Smoker= $("#get-a-quote-smoker").val(),gender= $("#get-a-quote-gender").val(),state= $("#get-a-quote-state").val(),us= $("#get-a-quote-us_citizen").prop("checked"),policyOwner= $("#get-a-quote-policy_owner").prop("checked"),replacement= $("#get-a-quote-not_replacing").prop("checked"),currentAnnualIncome=$("#get-a-quote-current-income").val(), successCallback = function() {       displaySuccessText("Thanks! We will be in touch soon!", "#contact-form"), $("#contact-form div:not(.success-text)").fadeOut();
	    };
	   submitContactInfo(email, phone, bestTimeToCall, calculatorData, needsCalculatorDemographicData, directQuoteDemographicData, coverage, term, successCallback,comments,name,firstName,dob,emailAddress,Smoker,gender,state,us,policyOwner,replacement,currentAnnualIncome,pathId);
	}
	else if (pathId == '1') {
	  // Calculate insurance needs path	
	   var needsCalculatorDemographicData = $(document.needsCalculatorDemographicsForm).serializeObject(), calculatorData = $(document.needsCalculatorForm).serializeObject(), directQuoteDemographicData = $(document.getQuoteDemographicsForm).serializeObject(), coverage = $(".calculated-coverage:first").text().replace("$", "").replace(",", ""), term = null, email = $("#contact-form-email").val(), phone = $("#contact-form-phone").val(), bestTimeToCall = $("#contact-form-best-time").val(), comments = $("#contact-form-comments").val(),name = $("#contact-form-name").val(), firstName= $("#calc-first-name").val(),dob = $("#calc-date-of-birth").val(),emailAddress= $("#calc-email-address").val(),Smoker= $("#calc-smoker").val(),gender= $("#calc-gender").val(),state= $("#calc-state").val(),us= $("#calc-us_citizen").prop("checked"),policyOwner= $("#calc-policy_owner").prop("checked"),replacement= $("#calc-not_replacing").prop("checked"),successCallback = function() {       displaySuccessText("Thanks! We will be in touch soon!", "#contact-form"), $("#contact-form div:not(.success-text)").fadeOut();
	    };
	   submitContactInfo(email, phone, bestTimeToCall, calculatorData, needsCalculatorDemographicData, directQuoteDemographicData, coverage, term, successCallback,comments,name,firstName,dob,emailAddress,Smoker,gender,state,us,policyOwner,replacement,null,pathId);
	}	
	else {
		var needsCalculatorDemographicData = $(document.needsCalculatorDemographicsForm).serializeObject(), calculatorData = $(document.needsCalculatorForm).serializeObject(), directQuoteDemographicData = $(document.getQuoteDemographicsForm).serializeObject(), coverage = $(".calculated-coverage:first").text().replace("$", "").replace(",", ""), term = null, email = $("#contact-form-email").val(), phone = $("#contact-form-phone").val(), bestTimeToCall = $("#contact-form-best-time").val(), comments = $("#contact-form-comments").val(),name = $("#contact-form-name").val(),successCallback = function() {       displaySuccessText("Thanks! We will be in touch soon!", "#contact-form"), $("#contact-form div:not(.success-text)").fadeOut();
	    };
	   submitContactInfo(email, phone, bestTimeToCall, calculatorData, needsCalculatorDemographicData, directQuoteDemographicData, coverage, term, successCallback,comments,name,null,null,null,null,null,null,null,null,null,null,pathId);
	}  
}

function getOriginalQuote(demographicsForm,formId) {
    var demographicData = $(demographicsForm).serializeObject(), coverage = $("#get-a-quote-requested_coverage").val(), 
    term = $("#get-a-quote-policy_term").val(), successCallback = function(response) {
        $(".best-class .price").text(response.bestClassPrice), $(".standard-class .price").text(response.standardClassPrice), 
        $("#preselected-coverage, #comparison-coverage").val(coverage), $("#preselected-term, #comparison-term").val(term), 
        populateRecommendedCoverageSlider($("#get-a-quote-requested_coverage").val()),
        populateRecommendedTermSlider($("#get-a-quote-policy_term").val()),
        populateUpdatedCoverageSlider($("#get-a-quote-requested_coverage").val()),
        populateUpdatedTermSlider($("#get-a-quote-policy_term").val()),
        
        $("#original-quote-originalCoverageAmnt").val($("#get-a-quote-requested_coverage").val()),
        $("#comparison-quote-selectedCoverageAmnt").val($("#get-a-quote-requested_coverage").val()),
        $("#original-quote-originalPolicyTerm").val($("#get-a-quote-policy_term").val()),
        $("#comparison-quote-selectedPolicyTerm").val($("#get-a-quote-policy_term").val()),
        
        changeSection("#quote-page");
        $(".quote.comparison").fadeOut(1, function() {
            $(".quote.add").fadeIn(1);
        });
    };
    getIKnowQuote(demographicData, successCallback, formId);
}

function getComparisonQuote(coverage, term) {
    var haveCalculatorData = "Recommended Coverage" === $("#quote-page .tab-heading span").text(), demographicData = haveCalculatorData ? $(document.needsCalculatorDemographicsForm).serializeObject() : $(document.getQuoteDemographicsForm).serializeObject(), successCallback = function(response) {
        $("#comparison-quote .best-class").find(".price").html(response.bestClassPrice), 
        $("#comparison-quote .standard-class").find(".price").html(response.standardClassPrice), 
        $("#comparison-quote .recommended-coverage .calculated-coverage").text("$" + numberWithCommas(coverage)), 
        $("#comparison-quote .recommended-coverage .calculated-term").text(term + " Years");
    };
    getQuote(demographicData, coverage, term, successCallback);
}

var calcResponse = null;
function calculatorSuccess2M() {
	...
	if (response.coverage == 100000) {
		$("#100KMessage").html("<b>Please note that $100,000 is the smallest coverage amount available through the web, if you feel you may need less coverage, please <a href=\"#contact-us\" class=\"open-contact-form\" onclick=\"openContactUsLayout()\" id=\"#mylifeCovered-contactUs-msg\"><u>contact us</u></a>.</b><br/>");
	} else {
		$("#100KMessage").html("");
	}
	...
	if ($("#childCount").val() == 0) {
		return  (response.coverage <= 0 && ($("#calculator-report h2").text("Congratulations!"), 
			    ...
			    $("#horizontalLine").hide(),
			    //$("#report-private-college-expenses span").html("$" + numberWithCommas(response.estimatedPrivateCollegeExpenses)), 
			    //$("#report-public-college-expenses span").html("+&nbsp;&nbsp;&nbsp;$" + numberWithCommas(response.estimatedPublicCollegeExpenses)), 
			    //$("#report-total-college-expenses span").html("$" + numberWithCommas(response.totalCollegeExpenses)), 
			    ...
			    //$("#report-number-of-children-requiring-funding span").html($("#childCount").val()), 
...
function calculatorSuccess(response) {
	...
	if (response.coverage >2e6) {
		msgPart = 'Our calculations indicate that your need is for $'+ numberWithCommas(response.coverage)+'.';
		msg = msgPart + ' Our online process offers a maximum coverage amount of $2,000,000. You can <a href=\"#\" onclick=\"calculatorSuccess2M()\" ><u>Get a Quote</u></a> for $2,000,000, or contact us to assist you with a larger amounts.';
	} 
	
	
	if ($("#childCount").val() == 0) {
		return response.coverage > 2e6 ? (void knockout2M(msg),$('#contact-form').show(),$('#contact-form').find(".hide-contact-fields").show(),$('#contact-form').find(".success-text").hide(),$('#contact-form').find(".error-text").hide()) : (response.coverage <= 0 && ($("#calculator-report h2").text("Congratulations!"), 
			    0 == $("#calculator-report h2").next("h3").length && $("#calculator-report h2").after("<h3>Looks like you don't need any life insurance.</h3>"), 
			    $("#calculator-report h3").addClass("margin-bottom-m"), $("#calculator-report .recommended-coverage").hide(), 
			    $("#coverage-analysis .tab-heading span").html("Needs Analysis"), $("#calculator-demographics").hide(), 
			    $("#coverage-analysis").removeClass().addClass("col-md-6 col-md-offset-3")), response.coverage > 0 && ($("#calculator-report h2").text("Your Recommended Coverage"), 
    ...
    b.prototype = {
        defaults: {
            ...
            definitions: {
                ...
                a: {
                    validator: "[A-Za-zÃ�ï¿½-Ã‘ï¿½Ã�ï¿½Ã‘â€˜Ãƒâ‚¬-ÃƒÂ¿Ã‚Âµ]",
                    cardinality: 1,
                    definitionSymbol: "*"
                },
                "*": {
                    validator: "[0-9A-Za-zÃ�ï¿½-Ã‘ï¿½Ã�ï¿½Ã‘â€˜Ãƒâ‚¬-ÃƒÂ¿Ã‚Âµ]",
                    cardinality: 1
                }
            },
...
+function(t) {
    ...
    var o = function(e, o) {
        this.options = o, this.$body = t(document.body), this.$element = t(e), this.$dialog = this.$element.find(".modal-dialog"), 
    ...
    o.VERSION = "3.3.6", o.TRANSITION_DURATION = 300, o.BACKDROP_TRANSITION_DURATION = 150, 
    o.DEFAULTS = {
        backdrop: !0,
        keyboard: !0,
        show: !0
        ...
        var i = this, s = t.Event("show.bs.modal", {
            relatedTarget: e
        });
        ...
        this.$dialog.on("mousedown.dismiss.bs.modal", function() {
            ...
            i.adjustDialog(), s && i.$element[0].offsetWidth, i.$element.addClass("in"), i.enforceFocus();
        ...
        this.$dialog.off("mousedown.dismiss.bs.modal"), t.support.transition && this.$element.hasClass("fade") ? this.$element.one("bsTransitionEnd", t.proxy(this.hideModal, this)).emulateTransitionEnd(o.TRANSITION_DURATION) : this.hideModal());
    ...
    t.fn.modal = e, t.fn.modal.Constructor = o, t.fn.modal.noConflict = function() {
        ...
        var i = t(this), s = i.attr("href"), n = t(i.attr("data-target") || s && s.replace(/.*(?=#[^\s]+$)/, "")), r = n.data("bs.modal") ? "toggle" : t.extend({
            remote: !/#/.test(s) && s
        }, n.data(), i.data());
...
// $(".main-section").not("#home-page-copy").hide();

$(".error-text").hide(), $(".success-text").hide(), 
$(window).on("hashchange", function() {
    console.log(location.hash);
    if ((location.hash != '') && (location.hash != '#legal-modal') && (location.hash != '#policy-modal') && (location.hash != '#financial-modal')) { 
        changeSection(location.hash); 
    }
}), changeAndScroll("#hero a", "#needs-calculator"), changeAndScroll(".open-needs-calculator", "#needs-calculator"),
changeAndScroll(".open-calculator-report", "#calculator-report"),
changeAndScroll(".open-get-started", "#needs-calculator"), changeAndScroll(".open-privacy-policy", "#privacy-policy"), 
changeAndScroll(".open-terms-of-service", "#terms-of-service"), changeAndScroll(".open-basics", "#basics"), 
changeAndScroll("#calls-to-action .misconceptions a", "#misconceptions"), changeAndScroll(".open-home-page-copy", "#home-page-copy"), 
changeAndScroll(".open-contact-form", "#contact-us"), changeAndScroll(".open-about-us", "#about-us"), 
$(".misconception-btn").on("click", function(e) {
    e.preventDefault();
    var pviz = $(this).parent("h4").next("p").is(":visible");
    $("#misconceptions p").slideUp(), $(".misconception-btn").html("+"), pviz || ($(this).parent("h4").next("p").slideDown(), 
    $(this).html("-"));
}), $("#calls-to-action .get-quote a").on("click", function(e) {
    // e.preventDefault(), $(".calculated-coverage").html("$" + numberWithCommas($("#cta-coverage").val())), 
    // $(".calculated-term").html(numberWithCommas($("#cta-term").val()) + " Years"), 
    // $("#quote-page .quote-progress").hide(), $("#get-a-quote-requested_coverage").val($("#cta-coverage").val()), 
    // $("#get-a-quote-policy_term").val($("#cta-term").val()), changeSection("#get-quote");
}), $(".open-contact-form").on("click", function(e) {
    $("#contact-us-intro-text").text("Fill out the form below and we'll be in touch.");
}), $("a.apply-now").on("click", function(e) {
    e.preventDefault();
}), $("#quote-compare").on("click", function(e) {
    e.preventDefault(), $(".quote.add").fadeOut(200, function() {
        $(".quote.comparison").fadeIn(500);
    });
    ...
    var value = $(this).val();
    $(this).prev("div").find(".value").html(numberWithCommas(value));
}), 
...
loadCookies("getQuoteDemographicsFormCookie", document.getQuoteDemographicsForm), 
$("#needs-calculator-form").on("keyup change", function() {
...
}), $("#get-a-quote-demographics-form").on("keyup change", function() {
    return saveCookies("getQuoteDemographicsFormCookie", document.getQuoteDemographicsForm);
    ...
    autoUnmask: !0,
    clearMaskOnLostFocus: !1
}), $("#get-a-quote-current-income").inputmask({    
    alias: "integer",
    integerDigits: "8",
    autoGroup: !0,
    groupSeparator: ",",
...
$("#get-a-quote-date-of-birth").inputmask({
    alias: "mm/dd/yyyy",
    autoUnmask: !0,
    clearMaskOnLostFocus: !1
}), $("#get-a-quote-current-income, #annual-income").keyup(function() {
    var income = $(this).val(), x20 = 20 * income, max = 25e3 * Math.floor(x20 / 25e3);
    max > 2e6 && (max = 2e6), $("#quote-page input[type='range'].coverage").attr("max", max);
}), $("#needs-calculator-demographics-form input[type='submit']").on("click", function(e) {
	pathId='1';
    if (e.preventDefault(), $("#needs-calculator-demographics-form").find(".error-text").hide(), $("#calc-first-name").val().length < 3) 
    	return void displayErrorText("Please enter your first name.", "#calculator-demographics");
    if ($("#calc-date-of-birth").val().length < 7) 
    	return void displayErrorText("Please enter a valid date of birth.", "#calculator-demographics");
    if (!isValidEmail($("#calc-email-address").val())) 
    	return void displayErrorText("Please enter a valid email address.", "#calculator-demographics");
    if	($("#rec-coverage").val()=="") 
    	return void displayErrorText("Please select no. of years.", "#calculator-demographics");
    
    if($("#calc-form-phone").val().length > 0) {
    	var regExp = new RegExp(/^\(\d{3}\) \d{3}-\d{4}$/);
    	if(!regExp.test($("#calc-form-phone").val())) {
    		return void displayErrorText("Please enter a valid phone number.", "#calculator-demographics");
    	}
    }
    
    getCalculatedQuote(document.needsCalculatorDemographicsForm,"#calculator-demographics",document.getElementById('calc-allow_marketing').checked,$("#calc-form-phone").val());
    
   /* var currentIncome = $("#report-annual-income span").text().replace("$", "").replace(",", ""), requestedCoverage = $("h3.calculated-coverage:first").text().replace("$", "").replace(",", "");
    return requestedCoverage > 20 * currentIncome ? void displayErrorText("You cannot apply online for a coverage amount exceeding 20 times your current income. To apply for this amount, <a href='#' class='open-contact-form secondary'>contact us.</a>", "#calculator-demographics") : $("#calc-state").val().length < 1 ? void displayErrorText("Please select a state.", "#calculator-demographics") : 1 != $("#calc-us_citizen").prop("checked") ? void knockout() : 1 != $("#calc-policy_owner").prop("checked") ? void knockout() : 1 != $("#calc-not_replacing").prop("checked") ? void knockout() : void getOriginalQuote(document.needsCalculatorDemographicsForm); */
}), 
$("#get-a-quote-demographics input[type='submit']").on("click", function(e) {
	pathId = '2';
    if (e.preventDefault(), $("#get-a-quote-demographics").find(".error-text").hide(), $("#get-a-quote-first-name").val().length < 3) 
    	return void displayErrorText("Please enter your first name.", "#get-a-quote-demographics");
    if ($("#get-a-quote-date-of-birth").val().length < 7) 
    	return void displayErrorText("Please enter a valid date of birth.", "#get-a-quote-demographics");
    if (!isValidEmail($("#get-a-quote-email-address").val())) 
    	return void displayErrorText("Please enter a valid email address.", "#get-a-quote-demographics");
    if ($("#get-a-quote-current-income").val().length < 1) 
    	return void displayErrorText("Please enter your current yearly income.", "#get-a-quote-demographics");
    if ($("#get-a-quote-current-income").val().length < 1) 
    	return void displayErrorText("Please enter your current yearly income.", "#get-a-quote-demographics");
    var currentIncome = $("#get-a-quote-current-income").val(), requestedCoverage = requestedCoverage = $("#get-a-quote-requested_coverage").val();
    getOriginalQuote(document.getQuoteDemographicsForm,"#get-a-quote-demographics");
    ...
    if($("#contact-form-phone").val().length > 0) {
   	...
   	return void sendContactForm(document.contactForm,pathId);
    
}), $("#comparison-coverage").on("change", function() {
    var coverage = $(this).val(), term = $("#comparison-term").val();
    getComparisonQuote(coverage, term);
}), $("#comparison-coverage").on("input change", function() {
    $("#comparison-quote .quote-coverage").text("$" + numberWithCommas($(this).val()));
}), $("#comparison-term").on("input change", function() {
    $("#comparison-quote .quote-term").text($(this).val() + " Years");
}), $("#comparison-term").on("change", function() {
    var coverage = $("#comparison-coverage").val(), term = $(this).val();
    getComparisonQuote(coverage, term);
}), $("#needs-calculator input[type='submit']").on("click", function(e) {
	
	if (e.preventDefault(), $("#needsCalculatorErrorMsgId").find(".error-text").hide(), isValidInputAmount($("#retirementSavings").val()) == 1) {
    	return void displayErrorText("Please enter your retirement savings.", "#needsCalculatorErrorMsgId");
	} else if(isValidInputAmount($("#retirementSavings").val()) == 2) {
    	return void displayErrorText("Please enter only positive values for retirement savings.", "#needsCalculatorErrorMsgId");
	
	} else if (isValidInputAmount($("#otherSavings").val()) == 1) {
    	return void displayErrorText("Please enter your other savings.", "#needsCalculatorErrorMsgId");
	} else if (isValidInputAmount($("#otherSavings").val()) == 2) {
	    	return void displayErrorText("Please enter only positive values for other savings.", "#needsCalculatorErrorMsgId");
	   
	} else if (isValidInputAmount($("#annualIncome").val()) == 1) {
	    	return void displayErrorText("Please enter your annual income.", "#needsCalculatorErrorMsgId");
	} else if (isValidInputAmount($("#annualIncome").val()) == 2) {
	    	return void displayErrorText("Please enter only positive values for annual income.", "#needsCalculatorErrorMsgId");
		
	} else if (isValidInputAmount($("#yearsIncomeProvided").val()) == 1) {
	    	return void displayErrorText("Please enter your years of income needed for beneficiaries.", "#needsCalculatorErrorMsgId");
	} else if (isValidInputAmount($("#yearsIncomeProvided").val()) == 2) {
	    	return void displayErrorText("Please enter only positive values for years of income needed for beneficiaries.", "#needsCalculatorErrorMsgId");
	} else if ($("#yearsIncomeProvided").val() > 30) {
    	return void displayErrorText("Please enter values between 0 to 30 for years of income needed for beneficiaries.", "#needsCalculatorErrorMsgId");
    
	} else if (isValidInputAmount($("#existingLifeInsurance").val()) == 1) {
	    	return void displayErrorText("Please enter your existing life insurance coverage.", "#needsCalculatorErrorMsgId");
	} else if (isValidInputAmount($("#existingLifeInsurance").val()) == 2) {
	    	return void displayErrorText("Please enter only positive values for existing life insurance coverage.", "#needsCalculatorErrorMsgId");
	    
	} else if (isValidInputAmount($("#finalExpenses").val()) == 1) {
	    	return void displayErrorText("Please enter your funeral expenses.", "#needsCalculatorErrorMsgId");
	} else if (isValidInputAmount($("#finalExpenses").val()) == 2) {
	    	return void displayErrorText("Please enter only positive values for funeral expenses.", "#needsCalculatorErrorMsgId");
	    
	} else if (isValidInputAmount($("#outstandingMortgage").val()) == 1) {
	    	return void displayErrorText("Please enter your outstanding mortgage.", "#needsCalculatorErrorMsgId");
	} else if (isValidInputAmount($("#outstandingMortgage").val()) == 2) {
	    	return void displayErrorText("Please enter only positive values for outstanding mortgage.", "#needsCalculatorErrorMsgId");
	    
	} else if (isValidInputAmount($("#otherOutstandingDebt").val()) == 1) {
	    	return void displayErrorText("Please enter your other outstanding debt.", "#needsCalculatorErrorMsgId");
	} else if (isValidInputAmount($("#otherOutstandingDebt").val()) == 2) {
	    	return void displayErrorText("Please enter only positive values for outstanding debt.", "#needsCalculatorErrorMsgId");
	    
	} else if (isValidInputAmount($("#estimatedInflationRate").val()) == 1) {
	    	return void displayErrorText("Please enter your estimated inflation rate.", "#needsCalculatorErrorMsgId");
	} else if (isValidInputAmount($("#estimatedInflationRate").val()) == 2) {
	    	return void displayErrorText("Please enter only positive values for estimated inflation rate.", "#needsCalculatorErrorMsgId");
	    
	} else if (isValidInputAmount($("#estimatedInvestmentReturn").val()) == 1) {
	    	return void displayErrorText("Please enter your estimated investment return.", "#needsCalculatorErrorMsgId");
	} else if (isValidInputAmount($("#estimatedInvestmentReturn").val()) == 2) {
	    	return void displayErrorText("Please enter only positive values for estimated investment return.", "#needsCalculatorErrorMsgId");
	}   
    var formData = $(document.needsCalculatorForm).serializeObject();
    getRecommendedCoverage(formData, calculatorSuccess);
});

function getCalculatedQuote(needsCalculatorDemographicsForm,formId,allowMarketing,phone) {
	if ($("#calc-requested_coverage").val() >= 2e6) {
		$("#quote-page").find(".calculated-coverage").html("$2,000,000");
		populateRecommendedCoverageSlider(2000000);
		populateUpdatedCoverageSlider(2000000);
	} else {
		$("#quote-page").find(".calculated-coverage").html('$'+numberWithCommas($("#calc-requested_coverage").val()));
		populateRecommendedCoverageSlider($("#calc-requested_coverage").val());
		populateUpdatedCoverageSlider($("#calc-requested_coverage").val());
	}
    var demographicData = $(needsCalculatorDemographicsForm).serializeObject(), coverage = $("#calc-requested_coverage").val(), 
    term = $("#rec-coverage").val(), successCallback = function(response) {
        $(".best-class .price").text(response.bestClassPrice), $(".standard-class .price").text(response.standardClassPrice), 
        ...
        populateUpdatedTermSlider($("#rec-coverage").val()),
    ...
    getIKnowQuote(demographicData, successCallback, formId,allowMarketing,phone);
}

function getQuoteContactUsErrorMsg() {
...
function resetCalculateInsTab(){
	$($("#needs-calculator .progress-bar li")).removeClass('current');
	$($("#needs-calculator .progress-bar li")[0]).addClass('current');
}
$("#calculate_coverage_needs").on("click", function(e) {
	resetInsuranceCovergaeNeedsForm();
	//resetCalculateInsTab();
});

$("#getStartedWithInsuranceNeeds1").on("click", function() {
	resetInsuranceCovergaeNeedsForm();
	//resetCalculateInsTab();
});

$("#getStartedWithInsuranceNeeds2").on("click", function() {
	resetInsuranceCovergaeNeedsForm();
	//resetCalculateInsTab();
});

$("#getStartedWithInsuranceNeeds3").on("click", function() {
	resetInsuranceCovergaeNeedsForm();
	//resetCalculateInsTab();
});

$("#get_quote_reset").on("click", function() {
	$.ajax({
		//beforeSend: resetCalculateQuoteForm(),
		url: 'calculateCoverageNeeds.do',
		success: function(data) {
			//document.forms['get-a-quote-demographics-form'].reset();
		},
		error: function(jqXHR, textStatus, errorThrown) {
			//document.forms['get-a-quote-demographics-form'].reset();
		}
...
function resetInsuranceCovergaeNeedsForm() {
	$.ajax({
		//beforeSend: resetCoverageForm(),
		url: 'calculateCoverageNeeds.do',
		success: function(data) {
			//resetCoverageForm();
		},
...
function resetCalculateQuoteForm(){
	document.forms['get-a-quote-demographics-form'].reset();
	document.forms['needs-calculator-demographics-form'].reset();	
}
function resetCoverageForm() {
	$(".error-text").hide();
	for(var i = 0; i < $("#childCount").size(); i++) {
		$("#children .child").slice(i).remove();
	}
	document.forms['needsCalculatorForm'].reset();
	changeSection("#needs-calculator");
}
...
function openContactUsLayout() {
	$('#contact-form').show();
	$('#contact-form').find(".hide-contact-fields").show();
	$('#contact-form').find(".success-text").hide();
	$('#contact-form').find(".error-text").hide();
	$("#contact-us-intro-text").html("Fill out the form below and we'll be in touch.");
}
function openBasics() {
	changeAndScroll(".open-basics", "#basics");
}

function openMisconceptions() {
	changeAndScroll(".open-misconceptions", "#misconceptions");
}
